...
@NamedNativeQuery(name = "getListOrderOfPersonByStatus", resultSetMapping = "orderInfoDto", query = "SELECT orders.id, orders.total_price, orders.size size_vn, product.name product_name, product.product_images ->> \"$[0]\" as product_img\n"
                +
                "FROM orders\n" +
                "INNER JOIN product \n" +
