...
import com.shopgiayonline.model.mapper.ProductMapper;
...
import com.shopgiayonline.repository.ConfigurationRepository;
import com.shopgiayonline.repository.OrderRepository;
import com.shopgiayonline.repository.ProductRepository;
import com.shopgiayonline.repository.ProductSizeRepository;
...
public class ProductServiceImpl implements ProductService {
    ...
    public PageableDto filterProduct(FilterProductReq req) {
        ...
        if (req.getSizes().size() > 0) {
            products = productRepository.searchProductBySize(req.getBrands(), req.getCategories(), req.getMinPrice(),
                    req.getMaxPrice(), req.getSizes(), limit, page.calculateOffset());

            totalItems = productRepository.countProductBySize(req.getBrands(), req.getCategories(), req.getMinPrice(),
                    req.getMaxPrice(), req.getSizes());
        } else {
            products = productRepository.searchProductAllSize(req.getBrands(), req.getCategories(), req.getMinPrice(),
                    req.getMaxPrice(), limit, page.calculateOffset());
            totalItems = productRepository.countProductAllSize(req.getBrands(), req.getCategories(), req.getMinPrice(),
                    req.getMaxPrice());
        }
        ...
        PageableDto result = new PageableDto(promotionService.checkPublicPromotion(products), totalPages,
                req.getPage());
    ...
    public PageableDto searchProductByKeyword(String keyword, Integer page) {
        ...
        // Get list product and totalItems
        List<ProductInfoDto> products = productRepository.searchProductByKeyword(keyword, limit,
    ...
    public PageableDto adminGetListProduct(String id, String name, String category, String brand, String order,
        ...
        List<Product> products = productRepository.adminGetListProduct(id, name, category, brand, order, direction,
                limit, pageInfo.calculateOffset());
